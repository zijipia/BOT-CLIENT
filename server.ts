...
import {
  createVoiceReceiver,
  voiceStreamManagers,
} from "./voice/VoiceStreamManager";
import { VoiceWebSocketServer } from "./voice/VoiceWebSocketServer";
  ...
  try {
    ...
    if (!user.bot) {
      res
        ...
        .json({
          error:
            "Token valid, nhưng đây không phải là Discord Bot Token (Bot flag = false).",
        });
      return;
    }
...
app.get(
  "/api/discord/guilds/:guildId/members",
  async (req: Request, res: Response) => {
    try {
      const token = req.headers.authorization;
      const { guildId } = req.params;
...
app.put(
  "/api/discord/channels/:channelId/messages/:messageId/reactions/:emoji/@me",
  async (req: Request, res: Response) => {
    try {
      const token = req.headers.authorization;
...
app.post("/api/discord/ai-bot-reply", async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      reply: response.text || "🤖 Xin chào! Bot đã nhận được tin nhắn của bạn.",
    });
  } catch (err: any) {
...
app.get(
  ["/api/ziplayer/stream", "/api/ziplayer/proxy-stream"],
  async (req: Request, res: Response) => {
    try {
      const query =
        (req.query.query as string) ||
        (req.query.url as string) ||
        (req.query.audioUrl as string);
      if (!query) {
      ...
      console.log(
        `[ZiPlayer Stream] Processing stream request for: "${query}"`,
      );
      ...
      try {
        ...
        if (
          ...
          console.log(
            `[ZiPlayer Stream] Extracting Readable stream for: "${track.title}" via player.save`,
          );
        ...
        console.log(
          `[ZiPlayer Stream] Plugin search did not match: ${searchErr?.message || searchErr}`,
        );
      }
      ...
      if (query.startsWith("http://") || query.startsWith("https://")) {
        console.log(
          `[ZiPlayer Stream] Direct audio URL fallback for: ${query}`,
        );
        ...
        if (!response.ok || !response.body) {
          res
            ...
            .send("Failed to fetch direct audio stream");
          return;
        }
...
const voiceWebSocketServer = new VoiceWebSocketServer(voiceWss);

// WebSocket Server for Real-Time Discord Gateway Bridge
const wss = new WebSocketServer({ noServer: true });

// Route WebSocket upgrades manually to prevent path conflicts
...
function createPlayerManager(session: ClientSession): PlayerManager {
  ...
  manager.on(
    "volumeChange",
    (player: Player, oldVolume: number, newVolume: number) => {
      safeSend(session.clientWs, {
        type: "PLAYER_EVENT",
        event: "volumeChange",
        guildId: player.guildId,
  ...
  manager.on("playerDestroy", (player: Player) => {
    ...
    const vManager = voiceStreamManagers.get(player.guildId);
    if (vManager) {
      vManager.destroy();
      voiceStreamManagers.delete(player.guildId);
    }
  });
...
function connectDiscordBot(session: ClientSession, rawToken: string) {
  ...
  client.on("voiceStateUpdate", (oldState, newState) => {
    ...
    const vManager = voiceStreamManagers.get(guildId);
    if (
      vManager &&
      oldState.channelId &&
      oldState.channelId !== newState.channelId
    ) {
      vManager.unsubscribe(oldState.id);
    }
  });
  ...
  client.on("shardDisconnect", (event: any, id: number) => {
    ...
    if (event?.code === 4014) {
      errorMessage =
        "Lỗi 4014: Disallowed Intent(s) - Token yêu cầu Intent nâng cao chưa được bật trong Discord Developer Portal.";
    } else if (event?.code === 4004) {
      errorMessage =
        "Lỗi 4004: Authentication Failed - Bot Token không hợp lệ.";
    }
  ...
  client.login(token).catch((err: any) => {
    ...
    if (/disallowed intents/i.test(errorMessage)) {
      errorMessage =
        "Lỗi 4014: Disallowed Intent(s) - Bật Privileged Gateway Intents nếu cần, hoặc kiểm tra lại cấu hình bot.";
...
async function handleJoinVoice(
  ...
  try {
    ...
    if (player.connection) {
      let vManager = voiceStreamManagers.get(guildId);
      if (!vManager) {
        vManager = createVoiceReceiver(player.connection);
        voiceStreamManagers.set(guildId, vManager);

        vManager.on("userStart", (userId: string) => {
          voiceWebSocketServer.broadcastJSON(guildId, {
            type: "user:start",
            userId,
          });
        });
        vManager.on("userEnd", (userId: string) => {
          voiceWebSocketServer.broadcastJSON(guildId, {
            type: "user:end",
            userId,
          });
        ...
        vManager.on(
          "pcm",
          ({ userId, pcm }: { userId: string; pcm: Buffer }) => {
            voiceWebSocketServer.broadcastPCM(guildId, userId, pcm);
          },
        );
      }
    }
    ...
    console.log(
      `[ZiPlayer] Real voice connection established: guild=${guildId}, channel=${channelId}`,
    );
    safeSend(session.clientWs, {
...
async function handleLeaveVoice(session: ClientSession, guildId: string) {
  try {
    ...
    if (vManager) {
      vManager.destroy();
      voiceStreamManagers.delete(guildId);
    }
...
async function handlePlayerPlay(
  ...
  try {
    ...
    if (!player) {
      player = await session.playerManager.create(guildId, {
        leaveOnEnd: false,
        selfDeaf,
        selfMute,
      } as any);
    ...
    console.log(
      `[ZiPlayer] play("${query}") requested by ${userId} in guild ${guildId}`,
    );
...
function handlePlayerControl(
  ...
  if (!player) {
    safeSend(session.clientWs, {
      ...
      event: "playerError",
      guildId,
      error: "Không có player đang hoạt động cho guild này",
    });
    return;
  }
  try {
...
async function start() {
  ...
  server.listen(PORT, "0.0.0.0", () => {
    console.log(
      `Discord Client Full-Stack Server listening on http://0.0.0.0:${PORT}`,
    );
  });
}
